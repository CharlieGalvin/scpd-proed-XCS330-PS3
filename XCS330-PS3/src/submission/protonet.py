...
import src.omniglot as omniglot
import src.util as util  # pylint: disable=unused-import
...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            latent_query = self._network(images_query)

            prototypes = []
            for class_idx in torch.unique(labels_support):
                class_mask = (labels_support == class_idx)
                prototypes.append(latent_support[class_mask].mean(dim=0))
            prototypes = torch.stack(prototypes)

            query_distances = torch.cdist(latent_query, prototypes)
            support_distances = torch.cdist(latent_support, prototypes)

            query_log_probs = -query_distances
            support_log_probs = -support_distances

            loss = F.cross_entropy(query_log_probs, labels_query)
            loss_batch.append(loss)

            accuracy_support = util.score(support_log_probs, labels_support)
            accuracy_query = util.score(query_log_probs, labels_query)
